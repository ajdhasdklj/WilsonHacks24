...
function craft() {
  ...
  for (var i = 0; i < recipes.length; i++) {
    if (checkRecipe(recipes[i][2])) {
      newItem=recipes[i][1];
      //Craft the new item!
      if (newItem === randomItem.id) {
        setTimeout(function () {
          // Redirect to a new page when the specific item is crafted
          window.location.href = 'win.html';
        }, 3000);
      }
      
      document.getElementById("result").innerHTML="<IMG src='http://www.101computing.net/mc/" + + recipes[i][1] + "-0.png'><br/>" + recipes[i][0];
       break;
    }
  }
  }
...
function selectCraftTable(cell_ID) {
    ...
    // If the craft table cell is empty, set the selected ingredient in the crafting table
    if (!craftTableCell.innerHTML.trim()) {
      craftTableCell.innerHTML = selectedCell.innerHTML;
      craftTable[cell_ID] = selectedIngredient;
    } else {
      // If the cell is not empty, remove the ingredient from the crafting table
